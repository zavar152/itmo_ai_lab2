...
public final class ASearchAlgorithm {
    public static <T> Optional<Node<T>> search(Graph<T> graph, T start, T finish) {
        System.out.println("Поиск А*: ");
        PriorityQueue<HeuristicNode<T>> queue = new PriorityQueue<>();
        Node<T> tNode1 = graph.getNode(start).get();
        queue.add(new HeuristicNode<>(tNode1, graph.getHeuristicOf(tNode1), 0));
        ...
        while (!queue.isEmpty()) {
            ...
            if (currentNode.getValue().equals(finish)) {
                ...
                neighbors.forEach(tNode -> {
                    ...
                    queue.add(new HeuristicNode<>(tNode, graph.getDistance(tNode, finalCurrentNode.getNode()), graph.getHeuristicOf(tNode)));
                });
